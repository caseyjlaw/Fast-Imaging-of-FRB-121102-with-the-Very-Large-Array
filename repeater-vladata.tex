...
The spectral power is calculated for periods between 0.8 and 80 s and a 95\% confidence bound is estimated from a sample of 100 random shuffles of the data. 

Figure \ref{fig:ls} shows that while no single period dominates the spectral power, there is excess power for periods shorter than {\color{red}xx s}. It is easy to generate excess power in a Lomb-Scargle periodogram with a non-stationary noise process. However, that tends to create spectral power on low frequencies \citep["red noise"][]{}. We verified that simulating nine bursts drawn from a simple rotational model would produce a strong, narrow peak in \ref{fig:ls}. If astrophysical, this excess power is consistent with the idea that the bursts imperfectly trace an underlying rotational period, as has been observed from many classes of neutron star. Examples include magnetars, which typically have wide pulse phase windows \citep{2007ApJ...663..497C} and glitches that change their periods \citep{2013Natur.497..591A}.
