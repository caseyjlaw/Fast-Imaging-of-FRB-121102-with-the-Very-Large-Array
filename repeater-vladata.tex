...
The calculation of a volumetric rate has been used to study classes of optical and high-energy transient \citep{2006ARA&A..44..507W}. \citet{2013Sci...341...53T} used this approach to estimate a rate of $10^{-3}$\ galaxy$^{-1}$\ yr$^{-1}$ by assuming an isotropic emitter in a Milky Way-type galaxy out to a characteristic redshift horizon of 0.9. This rate is high \citep[comparable to the rate of core-collapse supernovae;][]{2006Natur.439...45D}, which has been used to argue that FRBs are likely not associated with other classes of transient, such as GRBs or rarer SNe.

However, inferences based on this and related rates are difficult, since they are strongly dependent on the sensitivity of the telescopes. Here we reevaluate the FRB volumetric rate by assuming \frb\ is a prototype of the class. We also update the volumetric rate to use the latest projected FRB rate \citep[$2\times10^3\ \rm{sky}^{-1} \rm{day}^{-1}$\ at high Galactic latitudes and flux densities brighter than 1 Jy ms; ][]{2016arXiv161100458L, 2016MNRAS.460L..30C, 2016MNRAS.455.2207R}. 

We recast the estimate as volumetric rate of FRB sources by including beaming and repetition effects. The volumetric source rate is $R_{FRB} = R_p /(N_r \Omega_b V (z))$, where $R_p$\ is the projected FRB rate, $N_r$\ is the number of bursts per source, $\Omega_b$\ is the beaming fraction, and $V(z)$\ is the volume out to redshift $z$. 

The characteristic distance scale for FRBs can be inferred from the detectability of \frb. The projected FRB rate is defined for a fluence limit of 1 Jy ms which is detectable out to $z=2\ for the Parkes surveys that have detected the most FRBs. This distance is consistent with that inferred from the largest DMs observed \citep[$\sim1500$\ pc cm$^{-3}$;][]{2016MNRAS.460L..30C}. This assumes that DM is dominated by the IGM \citep[$DM\approx z\times900 \rm{pc}\ \rm{cm}^{-3}$\ for realistic ionization state;][]{2003ApJ...598L..79I,2004MNRAS.348..999I}, although other contributions are likely significant \citep{2014ApJ...780L..33M}. An independent argument for a characteristic distance of $z=2$\ is that it corresponds to the peak the in the cosmic star formation history \citep{2014ARA&A..52..415M} and most leading models for FRBs are driven by star formation \citep{2016MNRAS.462..941L,2016ApJ...826..226K,2017arXiv170104815K}.
