...
\label{sec:spec}
Figure \ref{fig:sgram} shows the dynamic spectra of all nine bursts detected by the VLA. We improved on the initial analysis presented in (CITE: LOC) by using a better calibration scheme and optimizing the detection signifnance over a range of DM. Table \ref{tab:spec} shows the improved burst parameters using this new scheme. 
...
The initial VLA detections were made by searching a DM grid that allowed inter-DM sensitivity losses up to roughly 10\%. For the VLA 3~GHz observing band, we estimated that trials could be separated by at most 10 pc cm$^{-3}$ \citep{2003ApJ...596.1142C}. We refined that analysis by maximizing the image SNR for DMs spaced by 1 pc cm$^{-3}$. The optimal DM values range from 552 to 572 pc cm$^{-3}$.

The uncertainty in the peak DM measurement is defined by the DM sensitivity loss curve and the significance of the burst. We estimate the DM likelihood distribution for each burst by using a generative model for the burst spectra \citep{2010arXiv1008.4686H}. The burst spectra as a function of frequency were fixed according to the broad Gaussian shape defined below (\S \ref{sec:specmod}) and made into dynamic spectra by applying a frequency-dependent delay. We assume a Gaussian error distribution for channels with low signal and error up to 100\% of the model amplitude for channels with high signal. This is consistent with the large channel-to-channel variations observed (potentially due to diffractive scintillation).
