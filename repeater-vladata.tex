...
\title{A Multi-telescope Campaign on \frb: Implications for the FRB Population}
...
\begin{abstract}
We present results of the coordinated observing campaign that made the first subarcsecond localization of a Fast Radio Burst, FRB 121102. Of the nine bursts detected by the Very Large Array at 3~GHz, three had simultaneous observing coverage at other observatories. Only one VLA burst was detected by another observatory, the Arecibo Observatory at 1.4~GHz. We use multi-observatory constraints and modeling of bursts seen only at 3~GHz to confirm earlier results showing that burst spectra are not well characterized by a power law. We find that burst spectra are characterized by a $\sim500$~MHz envelope and apparent radio energy as high as $10^{40}$\ erg. We attribute significant changes in the apparent dispersion between bursts to frequency-dependent profiles that add a systematic random to the estimate of DM by up to 1\%. We use \frb\ as a prototype of the FRB class to estimate a volumetric birth rate of FRB sources $R_{\rm{FRB}} \approx 5\times10^{-5}/N_r$\ Mpc$^{-3}$\ yr$^{-1}$, where $N_r$\ is the number of bursts per source over its lifetime. This rate is broadly consistent with models of FRBs from young pulsars or magnetars born in superluminous supernovae or long gamma-ray bursts, if the typical FRB repeats on the order of $10^3$\ times during its lifetime.
...
57643 & 2016-09-12/9:15:19 & 120 & 3 & 1\tablenotemark{d} \\ 
57644 & 2016-09-13/9:23:59 & 120 & 3 & 0  \\
57645 & 2016-09-14/9:20:23 & 120 & 3 & 1\tablenotemark{a,d} \\
57646 & 2016-09-15/9:16:29 & 120 & 3 & 1  \\
57647 & 2016-09-16/9:11:23 & 120 & 3 & 0  \\
57648 & 2016-09-17/8:59:20 & 120 & 3 & 1\tablenotemark{a,*,b,c,d} \\ 
57649 & 2016-09-18/8:59:27 & 120 & 3 & 1\tablenotemark{a,b,c,d} \\
...
\enddata
\tablecomments{Times are in UT. Frequency is the approximate center of the bandwidth.}
\tablenotetext{a}{Arecibo coverage at 1.4~GHz.}
\tablenotetext{*}{Arecibo detection at 1.4~GHz.}
\tablenotetext{b}{Effelsberg coverage at 4.5~GHz.}
\tablenotetext{c}{LWA coverage near 70~MHz.}
\tablenotetext{d}{AMI-LA coverage at 15~GHz.}
%\end{tabular}
%\tablenotetext{a}{}
%\label{tab:obs}
\end{deluxetable} 
...
Beginning in April 2016, VLA observations of \frb were simultaneously observed with the first station of the LWA\footnote{This LWA station is located at the VLA site and is the first to be deployed. It is sometimes referred to as ``LWA1'', but we use ``LWA'' here.} when possible. The LWA observations consisted of single phased-array beam centered on the location of \frb with two 4096-channel spectral windows spanning frequencies from 52.2--71.8~MHz and 68.2--87.8~MHz. The observations were taken in LWA's spectrometer mode with a channel size of 4.8~kHz and a sample time of 160~ms. 

Two of the VLA detected pulses, on 2016 September 17 and 18 had simultaneous coverage with LWA. The SEFD of the LWA is dependent on zenith angle, for these two bursts the estimated SEFD is 9.5 kJy and 8.7 kJy, respectively. The data were de-dispersed into time series with DMs ranging from 500 to 600 pc cm$^{-3}$\ using a step size of 1.0 pc cm$^{-3}$. All DMs were searched using PRESTO \citep{2001PhDT.......123R} with a boxcar matched-filtering width from the native time resolution up to 48 seconds. We also visually inspected the dedispersed time series for the DM range 557 to 560 pc cm$^{-3}$.

No dispersed pulses were found with significance greater than $10\sigma$\, equivalent to a flux density limit of $\sim60$\ Jy for a width of 160~ms.
The time resolution was set to be roughly equal to the dispersion smearing in a single channel for DM$\approx560$\ pc cm$^{-3}$...
...
\end{tabular}
\tablecomments{$S_{\rm{V}}$ is the measured circular polarization of the burst, which dominated by systematic effects. All error ranges represent 68\% confidence intervals.}
...
% ** signal or noise analysis offered by jim **
...
\begin{center}
 ...
 \end{minipage}
\caption{Spectra and best-fit model for nine bursts seen by the VLA from 2.5 to 3.5~GHz in the peak, dedispersed 5~ms integration. The solid line is a best-fit Gaussian model found through modeling. The typical flux density error per channel is 70~mJy, which is shown at the top right of each panel next to the MJD label.
\label{fig:spec}}
\end{center}
\end{figure*}
...
The initial VLA detections were made with a matched-filter approach that allowed inter-DM sensitivity losses up to roughly 10\% \citep[$\Delta \rm{DM}=10\ \rm{pc}\ \rm{cm}^{-3}$][]{2003ApJ...596.1142C}. The MCMC modeling is more sophisticated in that it considers a wider range of parameters and models both data and observing effects (frequency-time pixel boundaries) simultaneously.
...
% This is consistent with the idea that the DM measured at low temporal resolution (e.g., by the VLA or by single-dish observatories without coherent dedispersion) is determined by extrinsic (an electron column) dispersion and some spectrotemporal structure that changes between bursts. 
...
% ** expand on implication of spectral envelope in context of scintillation interpretation of FRB brightness **
...
\software{\emph{rtpipe}, \emph{realfast}}
