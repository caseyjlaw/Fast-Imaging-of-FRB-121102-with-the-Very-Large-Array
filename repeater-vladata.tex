...
The calculation of a volumetric rate has been used to identify classes of transient and their defining physical processes \citep{2006ARA&A..44..507W, 2012Sci...337..927G}. \citet{2013Sci...341...53T} used this approach to estimate an isotropic rate of $10^{-3}$\ galaxy$^{-1}$\ yr$^{-1}$ by assuming a characteristic redshift horizon of 0.9. This rate is high \citep[comparable to the rate of core-collapse supernovae;][]{2006Natur.439...45D}, which has been used to argue that FRBs are likely not associated with other classes of transient, such as GRBs or rarer SNe. 
...
We now reevaluate this rate using the properties of \frb\ as a prototype FRB. First, we recast this estimate as a beamed, volumetric rate, $R_{FRB} = R_p /(\Omega_b V (z))$, where $R_p$\ is the projected FRB rate, $\Omega_b$\ is the beaming fraction \citep[$\sim0.1$;][]{1998MNRAS.298..625T} and $V(z)$\ is the volume out to redshift $z$. Second, the projected FRB rate is now believed to be lower than initially believed \citep[$2\times10^3\ \rm{sky}^{-1} \rm{day}^{-1}$\ at high Galactic latitudes and flux densities brighter than 1 Jy ms; ][]{2016arXiv161100458L, 2016MNRAS.460L..30C, 2016MNRAS.455.2207R}. Including these effects, we estimate a volumetric rate of $6\times10^{-5} (0.1/\Omega_b)$\ Mpc$^{-3}$\ yr$^{-1}$, assuming a typical FRB horizon redshift of 0.9. For comparison, the LGRB and SLSN-I rates are orders of magnitude lower, near $10^{-8}$ Mpc$^{-3}$\ yr$^{-1}$ \citep{2012Sci...337..927G}.

The revised FRB rate is an improvement, but it still ignores multiple effects. The burst spectral structure discussed here shows that this estimate likely underestimates the FRB rate by missing bursts that fall outside the observing band (typically a few hundred MHz near 1.4~GHz). For a typical burst envelope width of 500~MHz centered at GHz-scale frequencies, this could underestimate the rate by a factor of a few. The horizon scale ($z=0.9$) is not well defined and can change the implied rate by an order of magnitude.

sensitivity is the biggest awkward factor!
an out: use luminosity distribution for absolute rate scale?

calculate number of repetitions and compare to Metzger limit


%DM dominated by the IGM \citep[$DM\approx z\times1200 \rm{pc}\ \rm{cm}^{-3}$ for fully ionized hydrogen or $DM\approx z\times900 \rm{pc}\ \rm{cm}^{-3}$ for more realistic ionization state;][]{2003ApJ...598L..79I,2004MNRAS.348..999I}. 
