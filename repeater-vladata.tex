...
The dynamic radio spectra (time versus frequency) were modeled using a Markov chain Monte Carlo (MCMC) method. We used the Goodman and Weare affine invariant sampler \citep{goodman2010ensemble} as implemented in \emph{emcee} \citep{2013PASP..125..306F}. The dynamic spectra were modeled prior to dispersion correction. The spectral structure was modeled as a broad Gaussian envelope as a function of channel ``ch'' as:
\begin{equation}
G(\rm{ch}) = \frac{\rm{amp}}{\sqrt{2\pi}} \exp{\left[-\frac{1}{2} \left(\frac{\rm{ch} - \rm{center}}{\sigma_{ch}}\right)^2\right]}
\end{equation}
\noindent where ``amp'' is in mJy per 5~ms integration, ``center'' is the Gaussian center channel, and $\sigma_{ch}$\ is its width. The arrival time was modeled with a cold plasma dispersion law with the arrival time in units of integrations of:
\begin{equation}
\rm{i} (\rm{ch}) = \rm{i0} + 4.1488\times10^{-3}\ \rm{DM} \left[f(\rm{ch})^{-2} - f(\rm{ch0})^{-2}\right]/(5 \rm{ms})
\end{equation}
\noindent where ``i0'' is the arrival time at the highest frequency, ``ch0'', and function ``f'' maps the channel to a frequency in units of GHz. Finally, the pulses were assumed to have an intrinsically square temporal width (parameter $W_{int}$). This 6-parameter model defined the intrinsic signal that was then distributed over a fixed time-frequency observing grid.
...
\begin{equation}
E_{\rm{int}} = (\rm{amp}\ 5\ \rm{ms}) ( \sigma_{ch}\ 2.355 4\ \rm{MHz}) 10^{-23} (4\pi L_d^2)\ \rm{erg}
\end{equation}
\noindent where the first term represents the burst fluence, the second term represents the integral of the burst spectrum, and $L_d$\ is the luminosity distance to the source.
