...
Four of the VLA bursts were observed simultaneously with Arecibo and AMI-LA, while two were observed simultaneously with all four observatories (Figure \ref{fig:multi}). Of these bursts, only one VLA burst (on MJD 57648) was simultaneously detected by another observatory (Arecibo). The VLA and Arecibo burst detections had significances of 25$\sigma$\ and 39$\sigma$, respectively. The VLA and Arecibo integrated flux densities were 111 mJy and 14 mJy, respectively.

Figure \ref{fig:sgram} shows a dynamic spectrum for the 57648 burst with the VLA and AO data aligned in time. The temporal resolution of the VLA data (5~ms) is much coarser than that of Arecibo, so data were resampled to the same temporal grid after correcting for barycentric and dispersion measure offsets. The dispersion correction for both data sets assumed a DM of 560.5 pc cm$^{-3}$, which was typical of the source during the observing campaign \citep{WEIRD}. The regridded dynamic spectrum has an apparent DM error that is evident either as a frequency-dependent time drift within the Arecibo observation or in the relative alignment of Arecibo and the VLA bursts. Both of these drifts are consistent with an effective DM (i.e., one that maximizes the detection S/N ratio) of $sim$562 pc cm$^{-3}$.
...
\begin{center}
 ...
 \caption{Composite dynamic specrum for the burst from \frb\ on MJD 57648 with data from the VLA and Arecibo observatories. Both data sets have been barycentered and corrected for an assumed dispersion measure of 560.5 pc $^{-3}$. The black line separates data from the VLA (2.5 to 3.5~GHz) and Arecibo (1.0 to 1.8~GHz). The Arecibo to VLA gain ratio is assumed to be 10.
 \label{fig:sgram}}
\end{center}
\end{figure*}
...
\begin{table*}
\caption{Properties of Bursts from \frb **update**}
\centering
\begin{tabular}{lccccccccc}
\hline
Date                & S$_{\rm{I,int}}$  & Image S/N & E$_{\rm{int}}$ & DM$_{\rm{opt}}$ & dt & S$_{\rm{I,peak}}$ & Center & FWHM & S$_{\rm{V}}$ \\
(MJD)               & (mJy)           &           & ($10^{38}$\ erg) & (pc cm$^{-3})$ & (ms) & (mJy) & (GHz)  & (MHz) & (mJy) \\ \hline
...
The top panel of Figure \ref{fig:dmmodel} shows the 95\% confidence intervals on the DM for all nine VLA bursts as a function of burst time. The burst DMs are not consistent with any single value and tend to be larger than the reference value of 560.5 pc cm$^{-3}$. The range and variance in DM observed by the VLA is similar to that reported for \frb\ in \citet{2016arXiv160308880S}. As noted before, this is consistent with the idea that the DM measured at low resolution (e.g., by the VLA) is defined by true dispersion and some intrinsic structure that changes between bursts. The VLA data demonstrate that this intrinsic DM effect is visible at frequencies near 3~GHz. Assuming a "true" DM of 560.5 pc cm$^{-3}$, the excess DM can be thought of as a frequency-dependent delay that is intrinsic to the burst with a delay rate of 1--2~ms GHz$^{-1}$.
